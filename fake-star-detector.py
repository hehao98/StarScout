...
def _validate_star(user):
    ...
    if (
        ...
        and (user["starredAt"].date() == user["updatedAt"].date() == user["createdAt"].date())
    ):
        return True
    else:
        return False
...
def read_from_mongo(uri, dbname, collection_name):
    ...
    for repo_name in github_dict:
        ...
        github_dict[repo_name]["fake_percentage"] = (
            github_dict[repo_name]["fake_stars"] / github_dict[repo_name]["total_stars"]) * 100

    # Sort by fake percentage
    sorted_github_dict = dict(sorted(github_dict.items(
    ), key=lambda item: item[1]["fake_percentage"], reverse=True))
